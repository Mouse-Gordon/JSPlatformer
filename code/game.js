...
Slug.prototype.act = function (step,level){
	  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.status = "lost";
    this.finishDelay = 1;
  }else if (type == "water" && this.status == null) {
    this.status = "lost";
	this.finishDelay = 1;
  }else if (type == "slug" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1; this.finishDelay = 1;
